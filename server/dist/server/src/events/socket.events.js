...
function registerSocketEvents(io) {
    io.on('connection', (socket) => {
        ...
        const userId = socket.handshake.auth?.userId || (0, uuid_1.v4)();
        ...
        socket.on('room:start', (data, callback) => {
            ...
            room.state = (0, game_1.createGameState)(data.code, gamePlayers);
        ...
        // ─── GAME: PLAY CARD ──────────────────────────────
        socket.on('game:play-card', (data, callback) => {
            const room = rooms.get(data.code);
            if (!room || !room.state) {
            ...
            if (state.drawStack > 0 && !(0, rules_1.canPlay)(card, topCard, state.chosenColor, state.drawStack)) {
                callback({ success: false, error: 'Must draw cards or stack a matching draw card' });
                return;
            }
            if (state.drawStack === 0 && !(0, rules_1.canPlay)(card, topCard, state.chosenColor)) {
            ...
            // Apply card effect
            const newState = (0, rules_1.applyCard)(state, card, data.chosenColor);
            ...
            if (room.state.drawStack > 0) {
                ...
                const canStack = nextPlayer.hand.some(c => (0, rules_1.canPlay)(c, nextTopCard, room.state.chosenColor, room.state.drawStack));
                if (!canStack) {
                    // Force draw
                    const result = (0, deck_1.drawCards)(room.state.deck, room.state.discardPile, room.state.drawStack);
                    ...
                    (0, rules_1.advanceTurn)(room.state);
                    const drawEvent = {
                        type: 'drew',
        ...
        // ─── GAME: DRAW CARD ──────────────────────────────
        socket.on('game:draw-card', (data, callback) => {
            const room = rooms.get(data.code);
            if (!room || !room.state) {
                callback({ success: false, error: 'Game not found' });
                return;
            ...
            const result = (0, deck_1.drawCards)(state.deck, state.discardPile, drawCount);
        ...
        // ─── GAME: CHALLENGE UNO ──────────────────────────
        socket.on('game:challenge-uno', (data) => {
            const room = rooms.get(data.code);
            if (!room || !room.state)
                return;
            ...
            if (target.hand.length === 1 && !target.saidUno) {
                const result = (0, deck_1.drawCards)(room.state.deck, room.state.discardPile, 2);
...
// ─── Helpers ───────────────────────────────────────────────
function broadcastRoomUpdate(io, code) {
    const room = rooms.get(code);
    if (!room)
        return;
...
function broadcastGameState(io, code, state) {
    const room = rooms.get(code);
    if (!room)
        return;
    // Send redacted state to each player
    for (const player of state.players) {
        const redacted = (0, game_1.redactState)(state, player.socketId);
        io.to(player.socketId).emit('game:state', redacted);
    }
}
function handlePlayerLeave(io, socket, code, leavingUserId) {
    const room = rooms.get(code);
    if (!room)
        return;
    socket.leave(code);
...
function formatCard(card) {
    ...
    if (card.value === 'wild-draw4')
        return '🌈 Wild +4';
    const colorEmoji = { red: '🔴', yellow: '🟡', green: '🟢', blue: '🔵' };
